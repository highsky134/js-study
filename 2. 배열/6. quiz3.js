...
while(true){
    var updateMember = prompt(`현재 멤버 [${groupTVXQ}] \n 수정할 이름을 입력하세요.`);
    
    if(groupTVXQ.includes(updateMember)){
        var newMember = prompt(`새로운 멤버 이름을 입력하세요`);
        var MemberIndex = groupTVXQ.indexOf(updateMember);
        // groupTVXQ[MemberIndex] = newMember;
        groupTVXQ.splice(MemberIndex,1,newMember);
    }else {        
