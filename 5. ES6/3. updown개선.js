...
// // 입력된 수를 체크 하여 범위와 정답을 확인하는 함수
// function checkAnswerAndUpdown(gameData) {
...
function runUpdownGame(gameData) {
    ...
    // gameData 디스트럭처링
    // 사본을 만드는거라 원본에 적용되지 않습니다.
    const { countDown, secret, answer, initCount } = gameData;
    ...
    while (true) {
        ...
        checkAnswerAndUpdown(gameData); // 수정해야됨 계속 돌아감
        if (secret === answer) {
